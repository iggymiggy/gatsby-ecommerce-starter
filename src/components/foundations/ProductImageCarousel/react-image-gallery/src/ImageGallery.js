...
class ImageGallery extends React.Component {
  ...
  handleImageLoaded(event, item) {
    ...
    const imageExists = this.loadedImages[item.original];
    const gatsbyImageExists = this.loadedImages[item.gatsbyImage];

    if (!imageExists && onImageLoad) {
      this.loadedImages[item.original] = true; // prevent from call again
      // image just loaded, call onImageLoad
      onImageLoad(event);
    } else if (!gatsbyImageExists && onImageLoad) {
      this.loadedImages[item.gatsbyImageExists] = true; // prevent from call again
      // image just loaded, call onImageLoad
      onImageLoad(event);
    }
  }
